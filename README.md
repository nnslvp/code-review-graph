...
> **This is a Ruby/Rails-focused fork** of [tirth8205/code-review-graph](https://github.com/tirth8205/code-review-graph).
> It brings **Ruby and Rails to first-class parity** with the other supported languages. Everything in the upstream
> documentation below still applies — see [Using this fork](#using-this-fork) for the one-line install change.

## Ruby & Rails support (this fork)

The graph understands Rails and common Ruby idioms, not just bare method definitions:

- **Object model** — classes, modules, methods, singleton methods / `class << self`, constants;
  inheritance (`INHERITS`) and mixins `include`/`extend`/`prepend` (`INCLUDES`/`EXTENDS`/`PREPENDS`), resolved to the real target.
- **Rails relationships** — ActiveRecord associations (`has_many`/`belongs_to`/`has_one`/`has_and_belongs_to_many`,
  honoring `class_name:` and `through:`) → `ASSOCIATES`; `delegate … to:` → `DELEGATES`;
  `ActiveModel::Attributes` (`attribute :name`) → typed accessors; `ActiveSupport::Concern` (`included do` /
  `class_methods do`) macros propagate to including classes; controllers/jobs/consumers/service objects as flow entry points.
- **Dependency injection (dry-rb)** — `register('key') { Const.new }` + `include Import['key']` → `DEPENDS_ON`,
  read from the explicit container registry (never guessed from naming), so no false dependencies.
- **Tests & coverage** — RSpec `describe SomeClass` anchors one precise `TESTED_BY` edge (test → class under test)
  instead of one noisy edge per `let`/`expect`/`before`; `tests_for` rolls a method query up to its class.
  SimpleCov `.resultset.json` line coverage is ingested per node.
- **Honest call resolution** — receiver-aware: constant receivers resolve to the right singleton method
  (and `.new` → `initialize`), `self` calls resolve within the class, and calls on unknown locals/injected
...
Point your install at the fork instead of the published package. For an MCP client:
...
```jsonc
{
  "mcpServers": {
    "code-review-graph": {
      "command": "uvx",
      "args": [
        "--from", "git+https://github.com/nnslvp/code-review-graph@ruby-rails-firstclass",
...
```bash
# run any command from the fork without a global install:
alias crg='uvx --from git+https://github.com/nnslvp/code-review-graph@ruby-rails-firstclass code-review-graph'
# or install it:
pipx install "git+https://github.com/nnslvp/code-review-graph@ruby-rails-firstclass"
```

Everything else — commands, daemon, environment variables, CI usage — works exactly as documented below.
